...
public class SlidingBlocks extends JPanel{ //jpanel disegna la griglia in un pannello dedicato
	...
	public String instance= " ";	//qua vanno inserite le celle ( cioË i fatti ) dopo che Ë stato fatto lo shuffle, NB non dimenticare di aggiornare questo file ogni volta che si esegue un movimento delle celle. Vedi funzione setInstance
	...
	public SlidingBlocks (int size, int dim, int mar) {
		...
		nbTiles = size*size -1 ; //perch√® non viene contato il tile bianco
		...
		//Finestra di scelta modalit‡
		if(JOptionPane.showConfirmDialog(null,"Vuoi avviare la versione automatizzata?", "Scelta Modalit‡", 1) == 0) {
			setGameFrame();
			gameOver = true;
			addMouse();
			newGame();
	...
	private void resolveAction() {
		...
		if(dir!=0) {
			...
			//Se dir Ë negativo va verso le posizioni negative, in caso contrario positive (immagina asse x e y)
	...
	private void newGame() {
		...
		while(!isSolvable()); //Fa fino a quando la griglia √® risolvibile
		gameOver=false;
		//		
		initDlv();
	...
	private void setInstance() {
		...
		for(AnswerSet a:answers.getAnswersets()){
			...
			while (st.hasMoreTokens()) { //per ognuno di questi answerset il programma deve essere in grado di orlare il file next-step con l'istanza aggiornata della mossa, ad esempio se canMoveDown sposti gi˘ quella casella e 
				String temp=st.nextToken();//ne calcoli le rispettive conseguenze. Questo va fatto per ognuno di questi fatti. Il tutto deve essere ricorsivo e deve continuare finchÈ non trova la condizione di stop.
...
//						System.out.println("Sinistra: " + temp );
	...
	// Solo la met√† delle permutazioni del puzzle sono risolvibili
	// Fino a quando un tile √® preceduto da un tile con un valore pi√π alto viene contato
	...
	private boolean isSolved() {
		if(tiles[tiles.length -1] !=0) // se il tile bianco non √® nella solved position ==> non solved
			return false;
	...
	//PARTE GRAFICA
	//setta propriet‡ grafiche del gioco
